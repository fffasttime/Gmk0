...
public:
    ...
    bool lookup(const Network::NNPlanes& features, Network::NN_Ouputs & result);

    // Insert a new entry.
    void insert(const Network::NNPlanes& features,
                const Network::NN_Ouputs& result);
...
private:
    ...
    struct Entry {
        Entry(const Network::NNPlanes& f, const Network::NN_Ouputs& r)
            : features(f), result(r) {}
        Network::NNPlanes features; // ~ 1KB
        Network::NN_Ouputs result;  // ~ 3KB
    };
